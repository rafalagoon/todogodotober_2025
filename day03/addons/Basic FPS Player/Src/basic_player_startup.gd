...
var tnt_preload = preload("res://tnt.tscn")
...
func throw ():
	if $Head/TNTFake.thrown:
		return

	$Head/TNTFake.thrown = true
	$Head/TNTFake.visible = false
	
	var tnt_pos:Vector3 = $Head/TNTFake.global_position
	var tnt_rot:Vector3 = $Head/TNTFake.global_rotation
	
	var tnt:RigidBody3D = tnt_preload.instantiate()
	
	get_parent().add_child(tnt)
	
	tnt.global_position = tnt_pos
	tnt.global_rotation = tnt_rot
	...
	tnt.apply_central_impulse(impulse)
